...
export const validateInviteCode = (code: string): boolean => {
  const dashed = /^[A-Z0-9]{4}(-[A-Z0-9]{4}){3}$/;
  const raw = /^[A-Z0-9]{16}$/;
  return dashed.test(code) || raw.test(code);
};
 ...
 */
/**
 * Format Invite Code ให้ตรงกับ backend (HEX: 0-9 + A-F)
 */
export const formatInviteCode = (input: string): string => {
  ...
  // ตัดความยาวสูงสุด 19 ตัว (16 + 3 dash)
